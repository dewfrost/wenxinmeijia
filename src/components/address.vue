<template>
  <div class="select-bg" v-show="showThree">
    <div class="p-box" v-on:click="close()"></div>
    <div class="selectAddressThree">
      <div class="title">所在地区 <i class="iconfont icon-close" v-on:click="close()"></i></div>
      <div class="select-div">
        <span v-bind:class="{now: addressSheng === '请选择'}" v-on:click="addressSheng = '请选择';addressShi = '请选择';addressQu = '请选择';showN(0)">{{ addressSheng }}</span>
        <span v-bind:class="{now: addressShi === '请选择'}" v-on:click="addressShi = '请选择';addressQu = '请选择';showN(1)" v-show="addressSheng !== '请选择'">{{ addressShi }}</span>
        <span v-bind:class="{now: addressQu === '请选择'}" v-on:click="addressQu = '请选择';showN(2)" v-show="addressShi !== '请选择'">{{ addressQu }}</span>
      </div>
      <transition-group name="list" tag="div" class="test">
        <ul v-for="item in addressMap" v-bind:key="item.name" class="ul1" v-bind:class="{hide: h1}">
          <li v-on:click="t1 = item.name;h1 = true;h2 = false;addressSheng = item.name;data.callBack([addressSheng,addressShi,addressQu])">{{ item.name }}</li>
          <ul v-for="city in item.city" v-bind:key="city.name" class="ul2" v-show="t1 === item.name" v-bind:class="{hide: h2}">
            <li v-on:click="t2 = city.name;h2 = true;h3= false;addressShi = city.name;data.callBack([addressSheng,addressShi,addressQu])">{{ city.name }}</li>
            <ul class="ul3" v-bind:key="city.name" v-show="t2 === city.name" v-bind:class="{hide: h3}">
              <li v-for="area in city.area" v-on:click="addressQu = area; data.callBack([addressSheng,addressShi,addressQu]);close()" :key="area.index">{{area}}</li>
            </ul>
          </ul>
        </ul>
      </transition-group>
...
export default {
    ...
    return {
      ...
      addressMap: [{
        ...
        'city': [{
          ...
          'ProID': 4,
          'CitySort': 22,
          'area': ['长治县', '城区', '襄垣县', '屯留县', '平顺县', '黎城县', '壶关县', '长子县', '武乡县', '沁县', '沁源县', '潞城市']
        }, {
          'CityID': 23,
          'name': '晋城市',
          'ProID': 4,
  ...
  beforeMount: function () {
    if (this.data.selectAdd) {
      this.addressSheng = this.data.selectAdd[0];
      this.addressShi = this.data.selectAdd[1];
      this.addressQu = this.data.selectAdd[2];
      this.t1 = this.data.selectAdd[0];
      this.t2 = this.data.selectAdd[1];
      this.t3 = this.data.selectAdd[2];
      this.showN(2);
    }
  }, // 挂载之前
  ...
  computed: {
    select () {
      return this.$store.state.outConfig.outAddress;
    }
  },
  methods: {
    showN: function (n) {
